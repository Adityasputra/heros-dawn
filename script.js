...
class CombatManager {
  ...
  static setupEventListeners() {
    // Combat controls
    document
      .getElementById("btnFindEnemy")
      ?.addEventListener("click", () => this.findEnemy());
    document
      .getElementById("btnFlee")
      ?.addEventListener("click", () => this.flee());
    ...
    document.querySelectorAll(".combat-btn").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        if (
          action &&
          GameState.combat.inBattle &&
          GameState.combat.playerTurn
        ) {
    ...
    document
      ...
      ?.addEventListener("click", () => this.endBattle());
  ...
  static findEnemy() {
    ...
    const possibleEnemies = this.enemies.filter(
      (enemy) =>
  ...
  static startBattle() {
    ...
    this.addLogEntry(
      `A wild ${GameState.combat.enemy.name} appears!`,
      "system"
    );
    ...
    NotificationManager.show(
      `Battle started against ${GameState.combat.enemy.name}!`,
      "warning"
    );
  }
  ...
  static playerPowerAttack() {
    ...
    this.addLogEntry(
      `You unleash a Power Strike on ${enemy.name} for ${damage} damage!`,
      "player"
    );
  ...
  static playerDefend() {
    this.addLogEntry(
      "You raise your guard, reducing incoming damage!",
      "player"
    );
    this.showBattleEffect("🛡️");
  ...
  static playerFireball() {
    ...
    const damage = Math.max(
      1,
      player.stats.magic + Math.floor(Math.random() * 20)
    );
    enemy.hp = Math.max(0, enemy.hp - damage);

    this.addLogEntry(
      `You cast Fireball at ${enemy.name} for ${damage} magic damage!`,
      "player"
    );
    this.showBattleEffect("🔥");
    this.animateAttack("player");
  ...
  static playerLightning() {
    ...
    const damage = Math.max(
      1,
      Math.floor(player.stats.magic * 1.5) + Math.floor(Math.random() * 25)
    );
    enemy.hp = Math.max(0, enemy.hp - damage);

    this.addLogEntry(
      `You strike ${enemy.name} with Lightning for ${damage} magic damage!`,
      "player"
    );
    this.showBattleEffect("⚡");
    this.animateAttack("player");
  ...
  static enemyTurn() {
    ...
    if (action === "power" && Math.random() > 0.7) {
      damage = Math.max(
        1,
        Math.floor(enemy.attack * 1.5) -
          Math.floor(player.stats.defense / 2) +
          Math.floor(Math.random() * 8)
      );
      this.addLogEntry(
        `${enemy.name} unleashes a powerful attack for ${damage} damage!`,
        "enemy"
      );
      this.showBattleEffect("💥");
    } else {
    ...
    if (GameState.combat.defending) {
      ...
      this.addLogEntry(
        `Your defense reduces the damage to ${damage}!`,
        "system"
      );
  ...
  static flee() {
    ...
    if (success) {
      ...
      this.addLogEntry(
        "Failed to flee! The enemy blocks your escape!",
        "system"
      );
  ...
  static updateCombatDisplay() {
    ...
    if (GameState.combat.inBattle) {
      ...
      battleStatus.textContent =
        'Ready for battle! Click "Find Enemy" to start.';
    }
  }
